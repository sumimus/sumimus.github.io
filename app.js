...
app.post('/result', (req, res) => {
  ...
  const URL = `https://maps.googleapis.com/maps/api/directions/json?origin="${route.origin}"&destination="${route.destination}"&key=AIzaSyCzCFYYlKlARy6WIglfkOtwqi8FeJFc6KM`;
  ...
  https.get(URL, function (response) {
    response.on('data', function(chunk) {
      ...
      r.routes[0].legs[0].steps.forEach(target => {
          const imageURL = `https://maps.googleapis.com/maps/api/streetview?size=640x480&heading=180&fov=120&location=${target.end_location.lat},${target.end_location.lng}&sensor=false&key=AIzaSyCzCFYYlKlARy6WIglfkOtwqi8FeJFc6KM`;
